...
float waveTime = 0; // 修改变量名称，避免与标准库冲突
Wave* wave_model;
...
void buildTessendorfWaveMesh(Wave* wave_model)
{
	...
	wave_model->buildField(waveTime);
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), reinterpret_cast<void*>(fieldArraySize));
...
void focus_callback(GLFWwindow* window, int focused)
{
	...
	{
		glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_NORMAL);
		printf("Cursor set to normal. Focus: %d\n", focused);
	}
}
